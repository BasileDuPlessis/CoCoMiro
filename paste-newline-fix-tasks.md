...
### **Task 2: Fix HTML-to-Text Parsing Bug** ✅ COMPLETED
...
**Implementation Steps**:
- [x] Update `parse_formatted_text()` in `src/canvas.rs` to handle self-closing `<br>` tags
- [x] Test that contenteditable HTML with `<br>` tags gets converted back to plain text with `\n` characters
- [x] Verify that editing and saving notes preserves line breaks
- [x] Add unit tests for HTML parsing edge cases
