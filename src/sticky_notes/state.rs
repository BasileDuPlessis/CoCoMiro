...
impl StickyNotesState {
    ...
    /// # Arguments
    /// * `handle` - The resize handle being used
    /// * `params` - Resize parameters containing mouse positions and original dimensions
    pub fn resize_to(&mut self, handle: ResizeHandle, params: ResizeParams) {
        if let Some(note_id) = self.selected_note_id {
            if let Some(note) = self.get_note_mut(note_id) {
                ...
                match handle {
                    ...
                    ResizeHandle::Top => Self::resize_top(
                        note,
                        delta_x,
                        delta_y,
                        params.original_width,
                    ...
                    ResizeHandle::Right => Self::resize_right(
                        note,
                        delta_x,
                        delta_y,
                        params.original_width,
                    ...
                    ResizeHandle::Bottom => Self::resize_bottom(
                        note,
                        delta_x,
                        delta_y,
                        params.original_width,
                    ...
                    ResizeHandle::Left => Self::resize_left(
                        note,
                        delta_x,
                        delta_y,
                        params.original_width,
    ...
    fn resize_top_right(
        ...
        note.width = (original_width + delta_x).max(50.0);
        note.height = (original_height - delta_y).max(40.0);
        note.y += old_height - note.height; // Move note to keep bottom-left corner fixed
    }
